...
- 2026-09-01 · Собрана таблица маршрутов-кандидатов OpenRouter (T-11, [#23](../../issues/23)):
  пять кандидатов плюс запасной, у каждого `structured_outputs` подтверждён **реальным
  вызовом**, а не карточкой; цена за 1000 изображений посчитана из `usage.cost` одного вызова на
  эталонном фото `item_15.jpg` (452×500). Разброс цены — 2,7×, разброс «во сколько токенов
  превращается одно и то же фото» — 3,2×. Таблица и метод —
  [research/ROUTES_meta1ex.md](research/ROUTES_meta1ex.md). Пункт про связку «фото + JSON-схема»
  в [about/KNOWN_ISSUES.md](about/KNOWN_ISSUES.md) закрыт окончательно; туда же добавлены две
  новые грабли (пустой ответ `gpt-5-nano` из-за reasoning, схема как промпт-токены на части
  маршрутов). Маршрут по умолчанию не выбран и `config.yaml` не тронут — это T-14
  ([#26](../../issues/26)).
